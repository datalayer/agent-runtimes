...
def _block(
    kind: str, children: list[dict[str, Any]], provenance: dict[str, Any], **extra: Any
) -> dict[str, Any]:
    return {
        "children": children,
        "direction": "ltr",
...
def _paragraph(words: str, provenance: dict[str, Any], fmt: int = 0) -> dict[str, Any]:
    return _block(
        "paragraph", [_text(words, fmt)], provenance, textFormat=fmt, textStyle=""
    )


def _bullets(lines: list[str], provenance: dict[str, Any]) -> dict[str, Any]:
    items = [
        _block("listitem", [_text(line)], provenance, value=index)
...
def _cell(words: str, header: bool, provenance: dict[str, Any]) -> dict[str, Any]:
    return _block(
        "tablecell",
        [_paragraph(words, provenance)],
        provenance,
        headerState=1 if header else 0,
        colSpan=1,
        rowSpan=1,
...
def _table(
    head: list[str], rows: list[list[str]], provenance: dict[str, Any]
) -> dict[str, Any]:
    body = [
        _block(
            "tablerow", [_cell(words, True, provenance) for words in head], provenance
        )
    ]
    body += [
        _block(
            "tablerow", [_cell(words, False, provenance) for words in row], provenance
        )
        for row in rows
    ]
...
def _analysis_output(
    analysis: dict[str, Any], provenance: dict[str, Any]
) -> dict[str, Any]:
    ...
    seed = "|".join(
        ...
        for key in ("evalset", "launch", "experiment", "run", "analysis")
    )
    digest = hashlib.sha1(  # an identifier, not a secret
        ...
        "outputs": [
            {
                ...
                "data": {ANALYSIS_MIME: analysis, "text/plain": summary},
                "metadata": {},
            }
        ],
...
def _from_ci(launch: dict[str, Any]) -> str:
    ...
    config = as_dict(launch.get("config"))
    git = as_dict(config.get("git"))
    if not git:
        return ""
    parts: list[str] = []
    if git.get("sha"):
...
def _usage_of(run: dict[str, Any]) -> dict[str, Any]:
    usage = as_dict(run.get("usage"))
    if not usage:
        metrics = as_dict(run.get("metrics"))
        usage = as_dict(metrics.get("pydantic_ai_usage"))
    return usage


def _case_rows(experiment: dict[str, Any]) -> list[dict[str, Any]]:
    metrics = as_dict(_latest_run(experiment).get("metrics"))
...
def _analysis_block(
    ...
    if kind == "scalar":
        ...
        shown = (
            ...
            and 0 <= value <= 1
            and "rate" in str(analysis.get("metric") or "")
            else str(value)
        )
        if "delta" in str(analysis.get("metric") or "") and isinstance(
            value, (int, float)
        ):
            shown = _signed_pct(value)
        return _paragraph(
            f"{analysis.get('name') or analysis.get('metric')}: {shown}", named
        )
    if kind == "table":
    ...
    if kind not in COMPONENT_ANALYSES:
        ...
        logger.info(
            "The report has an analysis of an unknown kind: %s", kind or "(none)"
        )
    ...
    *,
    ...
    if run:
        experiments = [
            ...
            if str(item.get("id") or "") == str(run.get("experiment_id") or "")
        ] or experiments
    ...
    version = (
        evalset.get("version")
        or launch.get("evalset_version")
        or run.get("evalset_version")
    )
    base = {
        "evalset": evalset_id,
        "version": version,
    ...
    if status:
        children.append(
            _paragraph(f"{status[:1].upper()}{status[1:]}.", on(), IS_ITALIC)
        )
    ranked = sorted(
        experiments,
    ...
    overview = (
        ...
        + (
            f"; {best.get('name')} leads at {_pct(best.get('latest_pass_rate'))}."
            if best
            else "."
        )
    )
    ...
    for index, left in enumerate(ranked):
        for right in ranked[index + 1 :]:
            delta = float(left.get("latest_pass_rate") or 0.0) - float(
                right.get("latest_pass_rate") or 0.0
            )
            pairs.append(
                [
                    str(left.get("name") or left.get("id")),
                    str(right.get("name") or right.get("id")),
                    _signed_pct(delta),
                ]
            )
    if pairs:
        children.append(
            _table(["Experiment", "Against", "Latest pass-rate delta"], pairs, on())
        )
    else:
        children.append(_paragraph("One experiment: nothing to compare against.", on()))
    for item in ranked:
        comparison = (
            item.get("latest_two_comparison")
            if isinstance(item.get("latest_two_comparison"), dict)
            else None
        )
        if comparison and comparison.get("run_ids"):
            ...
            children.append(
                _analysis_output(
                    summary, on(experiment=item.get("id"), analysis="latest_two")
                )
            )
        ...
        for item in ranked:
            for row in _case_rows(item):
                ...
                if name and name not in names:
                    names.append(name)
        head = ["Task", "Category", "Difficulty"] + [
            str(item.get("name") or item.get("id")) for item in ranked
        ]
        rows = []
        for name in names:
            first = next(
    ...
    failed_rows = (
        [row for row in cases if str(row.get("status") or "") == "failed"]
        if cases
        else [
    ...
    for row in failed_rows:
        mode = str(
            row.get("failure_mode") or row.get("failure_stage") or "unclassified"
        )
    ...
    if failed_rows:
        children.append(
            _bullets(
                [
                    ...
                    for mode, count in sorted(modes.items(), key=lambda pair: -pair[1])
                ],
                on(),
            )
        )
        ...
        if explained:
            children.append(
                _bullets(
                    [
                        f"{row.get('name')}: {row.get('explanation')}"
                        for row in explained[:12]
                    ],
                    on(),
                )
            )
    ...
    for result in report.get("evaluator_results") or []:
        if isinstance(result, dict) and result.get("latest_passed") is False:
            children.append(
                _paragraph(
                    f"Report gate {result.get('name')} failed: {result.get('summary') or ''}",
                    on(analysis=result.get("name")),
                )
            )
    ...
    for item in ranked:
        ...
        subject_line = f"{item.get('name') or item.get('id')}: agentspec {item.get('agent_spec_id') or 'unknown'}"
        if usage.get("model"):
            subject_line += f", model {usage.get('model')}"
            if usage.get("provider"):
                subject_line += f" ({usage.get('provider')})"
        children.append(_paragraph(subject_line, on(experiment=item.get("id"))))
    if not ranked:
    ...
    dataset = (
        evalset.get("dataset_ref")
        if isinstance(evalset.get("dataset_ref"), dict)
        else None
    )
    environments = sorted(
        {
            str(_latest_run(item).get("summary", {}).get("environment") or "")
            for item in ranked
        }
        - {""}
    )
    children.append(
        _bullets(
            [
                f"Dataset: {dataset.get('source_uid')} at revision {dataset.get('revision_uid')}"
                if dataset
                else "Dataset: none declared on the benchmark",
    ...
    for item in ranked:
        fetched, total = (
            int(item.get("runs_fetched") or 0),
            int(item.get("runs_total") or 0),
        )
        if total > fetched:
            limits.append(
                f"{item.get('name')}: {fetched} of {total} runs read; older runs are not in these numbers."
            )
    if any(name in {"llm_judge", "llm-judge"} for name in case_evaluators):
        limits.append(
            "Some tasks are graded by an LLM judge; its explanation is on each task, and a judge can be wrong."
        )
    if not cases:
        limits.append(
            "Task explanations and failure modes come from the task documents of a run; this report reads run metrics only."
        )
    ...
    children.append(
        _paragraph(
            "Every number above traces to a task and the notebook that produced it. To run the benchmark again:",
            on(),
        )
    )
    experiment_flags = (
        " ".join(f"--experiment-id {item.get('id')}" for item in ranked[:1])
        or "--experiment-id <experiment>"
    )
    children.append(
        _paragraph(
            f"datalayer evals runs launch {experiment_flags} --run-mode batch",
            on(),
            IS_CODE,
        )
    )
...
def _inline(children: Any) -> str:
    ...
    for child in children or []:
        ...
        if kind == "text":
            ...
            parts.append(
                f"[{_inline(child.get('children'))}]({child.get('url') or ''})"
            )
...
def _plain(node: Any) -> str:
    ...
    return " ".join(
        part for part in (_plain(child) for child in node.get("children") or []) if part
    )
    ...
    for index, item in enumerate(node.get("children") or [], start=1):
        ...
        words = _inline(
            [
                child
                for child in item.get("children") or []
                if not (isinstance(child, dict) and child.get("type") == "list")
            ]
        )
        if words:
...
def _table_markdown(node: dict[str, Any]) -> str:
    rows = [
        [
            _cell_markdown(_inline(cell.get("children")))
...
def lexical_blocks(state: dict[str, Any]) -> list[dict[str, Any]]:
    ...
    """
    root = as_dict(state.get("root")) if isinstance(state, dict) else {}
