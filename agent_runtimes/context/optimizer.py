...
agent = Agent(
    "ollama:gemma3:1b-it-qat", instructions="Be concise, reply with one sentence."
)


def print_message_content(part: Any) -> str:
...
def calculate_message_size(msg: Any) -> int:
...
def print_request_details(result: Any, turn_number: int) -> None:
...
def print_response_details(result: Any, turn_number: int) -> None:
...
def main() -> None:
    ...
    print(
        f"Total context size: {sum(calculate_message_size(msg) for msg in result3.all_messages())} bytes\n"
    )
