...
async def startup_info(request: Request) -> dict[str, Any]:
    ...
    try:
        ...
        if status.get("kernel_id"):
            sandbox_block["kernel_id"] = status["kernel_id"]
        if status.get("kernel_name"):
            sandbox_block["kernel_name"] = status["kernel_name"]
        if status.get("username"):
            sandbox_block["username"] = status["username"]
