...
JUPYTER_NOTEBOOK_COMPACTOR_INBROWSER_AGENTSPEC_0_0_1 = Agentspec(
    id="jupyter-notebook-compactor-inbrowser",
    version="0.0.1",
                name="Notebook Compactor (in browser)",
    description="Rewrite a notebook as short as it can be without changing what it computes, running entirely in the browser — no sign-in, no runtime, no server.",
    ...
    welcome_message="Hi! I make notebooks shorter without making them different — and I do it right here in your browser, with nothing installed and nobody signed in. Point me at the notebook on this page and I will merge what belongs together, remove dead code and stale outputs, and tell you exactly how many cells and lines went.",
    welcome_notebook=None,
    welcome_document=None,
    sandbox_variant=None,
    harness="vercel-ai",
...
3. Say what you changed, in the reader's terms, after you change it. 4. When something cannot be done, say why instead of producing a plausible
   substitute.""",
    ...
    tool_hooks=None,
    parameters=None,
    subagents=SubAgentsConfig(subagents=[SubAgentspecConfig(name='NotebookCompactor', description='Rewrites the notebook as short as it can be without changing what it computes. Use when a notebook has grown repetitive or full of dead code.', instructions='', ref='jupyter-notebook-compactor:0.0.1'), SubAgentspecConfig(name='CellFixer', description='Fixes a failing cell and proves the fix by running it. Use when a cell raises.', instructions='', ref='jupyter-cell-fixer:0.0.1'), SubAgentspecConfig(name='NotebookReproducer', description='Runs the notebook top to bottom on a fresh sandbox and reports what does not reproduce. Use before sharing a notebook with someone else.', instructions='', ref='jupyter-notebook-reproducer:0.0.1')], include_general_purpose=True, max_nesting_depth=2),
)
...
AGENTSPECS: Dict[str, Agentspec] = {
    ...
    "jupyter-notebook-compactor-inbrowser": JUPYTER_NOTEBOOK_COMPACTOR_INBROWSER_AGENTSPEC_0_0_1,
