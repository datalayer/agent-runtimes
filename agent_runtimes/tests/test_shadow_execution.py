...
class TestOpeningAShadow:
    async def test_it_is_a_root_already_running_with_one_open_attempt(self, store):
        execution = await open_shadow_execution(
            store, tool="execute_cell", task_uid="tsk_abc"
        )
        ...
        assert (
            ...
            == f"att_{execution.execution_id.removeprefix('exec_')}_1"
        )

    async def test_a_retried_call_finds_the_same_shadow_rather_than_a_second_one(
        self, store
    ):
        first = await open_shadow_execution(
            store, tool="execute_cell", task_uid="tsk_retry"
        )
        second = await open_shadow_execution(
            store, tool="execute_cell", task_uid="tsk_retry"
        )
    ...
    async def test_two_different_tasks_are_two_different_shadows(self, store):
        ...
        other = await open_shadow_execution(
            store, tool="execute_cell", task_uid="tsk_2"
        )
    ...
    async def test_the_idempotency_key_is_the_task_uid_scoped_to_shadows(self, store):
        execution = await open_shadow_execution(
            store, tool="execute_cell", task_uid="tsk_key"
        )
...
class TestClosingAShadow:
    async def test_completed_closes_the_execution_and_its_attempt(self, store):
        execution = await open_shadow_execution(
            store, tool="execute_cell", task_uid="tsk_ok"
        )
        ended = await close_shadow_execution(
            store, task_uid="tsk_ok", outcome="completed"
        )
        assert ended is not None
        assert ended.status is ExecutionState.COMPLETED
        [attempt] = await store.attempts(execution.execution_id)
        assert (
            attempt.ended_at is not None and attempt.state is ExecutionState.COMPLETED
        )
    ...
    async def test_failed_and_cancelled_map_to_their_own_terminal_states(self, store):
        ...
        failed = await close_shadow_execution(
            store, task_uid="tsk_fail", outcome="failed"
        )
        ...
        cancelled = await close_shadow_execution(
            store, task_uid="tsk_cancel", outcome="cancelled"
        )
    ...
    async def test_a_task_uid_naming_no_shadow_answers_none_not_a_failure(self, store):
        ...
        assert (
            await close_shadow_execution(
                store, task_uid="tsk_never_shadowed", outcome="completed"
            )
            is None
        )

    async def test_closing_twice_is_answered_from_the_terminal_row_it_already_is(
        self, store
    ):
        await open_shadow_execution(store, tool="execute_cell", task_uid="tsk_twice")
        first = await close_shadow_execution(
            store, task_uid="tsk_twice", outcome="completed"
        )
        second = await close_shadow_execution(
            store, task_uid="tsk_twice", outcome="completed"
        )
