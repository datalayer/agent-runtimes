...
class TestASeatIsNamedByItsTeam:
    def test_every_catalogued_seat_is_found_again_by_its_agent_id(self) -> None:
        for team in list_team_specs():
            ...
            assert (
                ...
                and supervisor.member is None
                and supervisor.team.id == team.id
            )
    ...
    def test_a_seat_is_bound_over_a2a_with_no_endpoint_so_its_worker_is_brought_up(
        self,
    ) -> None:
        team = _team(a=())
        root, child = supervisor_binding(team), member_binding(team, team.agents[0])
        assert (root.agent_id, root.protocol, root.endpoint) == (
            "team:t/supervisor",
            AgentProtocol.A2A,
            None,
        )
        assert (child.agent_id, child.protocol, child.endpoint) == (
            "team:t/a",
            AgentProtocol.A2A,
            None,
        )
...
class TestTheMembersRunInTheOrderTheirDependenciesGive:
    def test_a_sequential_team_runs_one_member_at_a_time(self) -> None:
        ...
        assert [[member.id for member in layer] for layer in schedule(team)] == [
            ["a"],
            ["b"],
            ["c"],
            ["d"],
    ...
    def test_every_catalogued_scheduled_team_has_an_order(self) -> None:
        scheduled = [
            team for team in list_team_specs() if team.execution_mode in SCHEDULED_MODES
        ]
        assert scheduled
        for team in scheduled:
            assert sorted(
                member.id for layer in schedule(team) for member in layer
            ) == sorted(member.id for member in team.agents)
...
class TestASeatIsRegisteredFromWhatItNames:
    def test_a_seat_referencing_an_agentspec_is_registered_by_it(self) -> None:
        ...
        assert (
            fields["agent_spec_id"] == analyst.ref.split(":", 1)[0]
            and "system_prompt" not in fields
        )
        supervisor = registration(TeamSeat(jupyter))
        assert jupyter.supervisor is not None
    ...
    def test_a_seat_naming_none_is_registered_from_its_own_fields(self) -> None:
        team = next(
            team
            for team in list_team_specs()
            if not any(member.ref for member in team.agents)
        )
        for member in team.agents:
            fields = registration(TeamSeat(team, member))
            assert (
                "agent_spec_id" not in fields and member.goal in fields["system_prompt"]
            )
            assert fields.get("model", "") == member.model and fields.get(
                "tools", []
            ) == list(member.tools)
            if member.mcp_server:
                assert fields["selected_mcp_servers"] == [
                    {"id": member.mcp_server, "origin": "catalog"}
                ]
        supervisor = registration(TeamSeat(team))
        assert team.supervisor is not None
        assert (
            team.supervisor.goal in supervisor["system_prompt"]
            or supervisor["system_prompt"] == team.supervisor.instructions
        )

    def test_a_supervisor_that_routes_is_given_its_members_as_seats_to_ask_for(
        self,
    ) -> None:
        jupyter = get_team_spec("jupyter")
        ...
        assert [seat["ref"] for seat in seats] == [
    ...
    def test_the_seats_a_supervisor_is_registered_with_are_subagents_it_asks_the_control_plane_for(
        self,
    ) -> None:
        ...
        spec = _with_subagents(
            get_agent_spec(fields["agent_spec_id"]),
            request,
            "a2a-team-jupyter-supervisor",
        )
        [capability] = [
            one
            for one in build_capabilities_from_agent_spec(
                spec, agent_id="a2a-team-jupyter-supervisor"
            )
            if isinstance(one, SubagentsCapability)
        ]
        assert [
            definition.a2a and definition.a2a.spec_id
            for definition in capability.subagents
        ] == [seat_agent_id("jupyter", member.id) for member in jupyter.agents]
        assert capability.include_general_purpose is False
        # An agent created from no spec is still given the subagents it names.
        bare = _with_subagents(
            None, CreateAgentRequest(name="bare", subagents=request.subagents), "bare"
        )
