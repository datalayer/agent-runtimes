...
class GatedAgent(BaseAgent):
    ...
    async def stream(
        ...
        self.histories.append(
            [dict(message) for message in context.conversation_history]
        )
...
def client(tmp_path: Any, monkeypatch: Any) -> Iterator[TestClient]:
    monkeypatch.setattr(
        state_store, "_store", SqliteProtocolStateStore(tmp_path / "state.sqlite")
    )
    app = FastAPI()
...
@contextmanager
def session(
    client: TestClient, agent_id: str, agent: GatedAgent
) -> Iterator[tuple[Wire, str, dict[str, Any]]]:
...
def test_the_agent_says_it_speaks_the_extension(client: TestClient) -> None:
    with session(client, "gated-card", GatedAgent()) as (_, _, initialized):
        assert (
            EXTENSION_URI
            in initialized["agentCapabilities"]["_meta"]["datalayer"]["extensions"]
        )


def test_a_paused_turn_keeps_its_conversation_and_answers_with_the_checkpoint(
    client: TestClient,
) -> None:
    agent = GatedAgent()
    with session(client, "gated-pause", agent) as (wire, session_id, _):
        prompted = wire.send(
            "session/prompt", _prompt(session_id, "Profile the notebook")
        )
        assert agent.reached.wait(5)
        wire.notify(
            "session/cancel",
            {"sessionId": session_id, "_meta": {"datalayer": {"pause": True}}},
        )
    ...
    checkpoint = asyncio.run(
        ProtocolStateCheckpointStore("exec_acp").get(checkpoint_id)
    )
...
class SpendingAgent(GatedAgent):
    ...
    async def stream(
        ...
        yield StreamEvent(
            type="done", data={"usage": {"input_tokens": 21, "output_tokens": 8}}
        )
...
def test_a_turn_answers_with_what_it_spent(client: TestClient) -> None:
    ...
    assert spent_of(answered["result"]["_meta"]) == Usage(
...
def test_a_steer_reaches_the_running_turn(client: TestClient) -> None:
    ...
    with session(client, "gated-steer", agent) as (wire, session_id, _):
        ...
        wire.notify(
            "_datalayer/steer",
            {"sessionId": session_id, "instructions": "Check the plots"},
        )
...
def test_a_turn_naming_the_checkpoint_resumes_from_it(client: TestClient) -> None:
    kept = asyncio.run(
        ProtocolStateCheckpointStore("exec_acp").create_checkpoint(
    ...
    with session(client, "gated-resume", agent) as (wire, session_id, _):
        _, answered = wire.request(
            "session/prompt",
            _prompt(session_id, "Carry on", checkpoint={"checkpointId": kept.id}),
        )
    assert answered["result"]["stopReason"] == "end_turn"
...
def test_a_checkpoint_this_runtime_does_not_keep_is_refused(client: TestClient) -> None:
    ...
    with session(client, "gated-unknown", agent) as (wire, session_id, _):
        _, answered = wire.request(
            "session/prompt",
            _prompt(
                session_id, "Carry on", checkpoint={"checkpointId": "ckpt_nowhere"}
            ),
        )
    assert (
        answered["error"]["code"] == -32602
        and "ckpt_nowhere" in answered["error"]["message"]
    )
