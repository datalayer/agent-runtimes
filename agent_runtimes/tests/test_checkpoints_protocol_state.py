...
async def test_a_checkpoint_is_read_back_by_another_process(path: Any) -> None:
    made = await _store(path).create_checkpoint(
        "paused", turn=2, messages=MESSAGES, metadata={"attempt": "att_1"}
    )
    again = await _store(path).get(made.id)
    assert again is not None
    assert (again.label, again.turn, again.messages, again.metadata) == (
        "paused",
        2,
        MESSAGES,
        {"attempt": "att_1"},
...
async def test_newest_first_and_pruned_within_its_own_execution(path: Any) -> None:
    ...
    for turn in range(1, 5):
        await mine.create_checkpoint(
            f"turn-{turn}", turn=turn, messages=MESSAGES, max_checkpoints=3
        )
    assert [checkpoint.label for checkpoint in await mine.list_all()] == [
        "turn-4",
        "turn-3",
        "turn-2",
    ]
...
@pytest.mark.asyncio
async def test_the_middleware_and_the_tools_work_over_it_across_processes(
    path: Any,
) -> None:
    ...
    config = CheckpointConfig(
        enabled=True, frequency="every_turn", store="protocol_state"
    )
    ...
    [saved] = [
        checkpoint
        for checkpoint in await restarted.list_checkpoints()
        if checkpoint.label == "before-modelling"
    ]
    answer = await rewind_to_tool_fn(restarted)["function"](
        saved.id, "the plots were wrong"
    )
    assert "Rewinding to 'before-modelling'" in answer
    pending = restarted.take_pending_rewind()
    assert pending is not None and pending.id == saved.id
