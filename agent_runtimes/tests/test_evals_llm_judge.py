...
def test_a_passing_verdict_scores_and_explains():
    judge = _judge(
        {
            "score": 0.95,
            "passed": True,
            "explanation": "The count matches.",
            "failure_mode": "",
        }
    )
    outcome = run_case_evaluators(
        output="There are 2000 rows.",
        expected="2000",
        evaluators=[
            {
                "name": "llm-judge",
    ...
    assert (
        "Expected output:\n2000" in judge.prompts[0]
        and "There are 2000 rows." in judge.prompts[0]
    )
...
def test_a_failing_verdict_names_how_it_failed():
    ...
    outcome = run_case_evaluators(
        output="0",
        expected="157",
        evaluators=[{"name": "llm_judge", "arguments": {"_judge": judge}}],
    )
...
def test_the_threshold_decides_and_an_unknown_mode_is_other():
    judge = _judge(
        {"score": 0.6, "passed": True, "explanation": "close", "failure_mode": "sloppy"}
    )
    outcome = run_case_evaluators(
        output="x",
        expected="y",
        evaluators=[
            {"name": "llm-judge", "arguments": {"_judge": judge, "threshold": 0.7}}
        ],
    )
...
def test_a_judge_that_does_not_answer_json_or_cannot_be_asked_is_a_scorer_failure():
    outcome = run_case_evaluators(
        ...
        evaluators=[
            {
                "name": "llm-judge",
                "arguments": {"_judge": _judge("I think it is fine.")},
            }
        ],
    )
    ...
    outcome = run_case_evaluators(
        output="x",
        expected="y",
        evaluators=[{"name": "llm-judge", "arguments": {"_judge": down}}],
    )
...
def test_without_any_judge_the_case_says_so(monkeypatch):
    ...
    outcome = run_case_evaluators(
        output="x", expected="y", evaluators=[{"name": "llm-judge"}]
    )
    assert (
        outcome["passed"] is False
        and "no judge is configured" in outcome["evaluators"][0]["reason"]
    )
...
def test_the_default_judge_is_configurable(monkeypatch):
    ...
    try:
        outcome = run_case_evaluators(
            output="x", expected="x", evaluators=[{"name": "llm-judge"}]
        )
...
def test_the_http_judge_asks_inference_as_the_person(monkeypatch):
    ...
    def handler(request: httpx.Request) -> httpx.Response:
        ...
        return httpx.Response(
            ...
            json={
                "success": True,
                "data": {
                    "response": '{"score": 0.9, "passed": true, "explanation": "yes"}'
                },
            },
        )
    ...
    assert (
        seen["url"] == "https://inference.example/api/ai-inference/v1/chat/completions"
    )
    assert seen["auth"] == "Bearer dla_tmp_user"
    assert (
        seen["body"]["model"] == "claude-sonnet-4-6" and seen["body"]["stream"] is False
    )
    ...
    cases = [
        {
            ...
            "inputs": {"prompt": "?"},
            "expected_output": "157",
            "evaluators": [{"name": "llm-judge", "arguments": {"_judge": judge}}],
        }
    ]
