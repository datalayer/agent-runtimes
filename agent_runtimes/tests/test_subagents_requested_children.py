    ...
    async def child(self, *, ending: LifecycleEvent = LifecycleEvent.COMPLETE) -> None:
        ...
        child = await store.create(
            an_execution(
                ...
                root_execution_id="exec_parent",
            )
        )
        attempt = await store.record_attempt(
            an_attempt(child, attempt_id="att_child_1")
        )
        ...
        for observation in (
            ...
            Observation.progress(
                "Found three facts", data={"phase": "text", "text": "Found three facts"}
            ),
            Observation.produced(
                answer_artifact(child, attempt, "The facts."),
                data={"text": "The facts."},
            ),
        ):
            await store.record(
                child.execution_id, observation, attempt_id=attempt.attempt_id
            )
        failure = (
            OrchestrationError(
        ...
        await store.record(
            child.execution_id,
            Observation.moved(ending, error=failure),
...
def phases(monkeypatch: pytest.MonkeyPatch) -> list[tuple[str, str, dict[str, Any]]]:
    ...
    def emit(
        self: Any, subagent_name: str, tool_call_id: Any, phase: str, **payload: Any
    ) -> None:
        ...
        return ModelResponse(
            parts=[
                ToolCallPart(
                    ...
                    args={"subagent_name": subagent, "task": task},
                )
            ]
        )
...
class TestTheRunKnowsItsExecution:
    def test_a_transport_puts_the_execution_its_delegation_names_on_the_run(
        self,
    ) -> None:
        delegation.enter_execution({"datalayer": {"execution": dict(PARENT)}})
    ...
    def test_the_delegation_names_the_account_beside_the_execution(self) -> None:
        ...
        assert (
            meta is not None and meta["datalayer"]["execution"]["accountUid"] == "org-1"
        )
...
class TestAnOrchestratedRunAsksForItsChildren:
    ...
    async def test_delegating_to_an_a2a_subagent_asks_the_control_plane(
        ...
        capability = SubagentsCapability(
            subagents=[
                SubagentDefinition(
                    name="researcher", description="Facts", a2a=RESEARCHER
                )
            ],
        ...
        result = await Agent(
            _delegating("researcher", "Find the facts"), capabilities=[capability]
        ).run("Research")

        assert result.output == "The facts."
        [(command, account_uid)] = plane.delegated
        assert (command.parent_execution_id, account_uid, plane.tokens) == (
            "exec_parent",
            "org-1",
            ["tok-exec"],
        )
        ...
        said = [
            (phase, payload.get("text") or payload.get("state"))
            for _, phase, payload in phases
        ]
    ...
    @pytest.mark.asyncio
    async def test_a_child_over_acp_is_asked_for_as_one_over_a2a(
        ...
        answer = await request_child(
            PARENT, "reviewer", reviewer, "Review the facts", emit=lambda *a, **k: None
        )

        assert answer == "The facts."
        [(command, account_uid)] = plane.delegated
        assert (command.agent, command.parent_execution_id, account_uid) == (
            reviewer,
            "exec_parent",
            "org-1",
        )
    ...
    @pytest.mark.asyncio
    async def test_a_run_without_its_executions_token_cannot_ask(
        self, plane: FakePlane
    ) -> None:
    ...
    @pytest.mark.asyncio
    async def test_a_worker_that_reaches_or_launches_a_child_itself_is_refused(
        self,
    ) -> None:
        _orchestrated()
        ...
        with pytest.raises(ChildNotRequested, match="delegate a task"):
            await relay_a2a_task(
                A2ARemoteAgent(
                    name="researcher", url="http://agent.test", launch="remote"
                ),
                "Find the facts",
                context_id="ctx",
                emit=lambda *a, **k: None,
    ...
    @pytest.mark.asyncio
    async def test_an_in_process_subagent_stays_in_the_run(
        self, plane: FakePlane, phases: list[Any]
    ) -> None:
        ...
        result = await Agent(
            _delegating("writer", "Write it up"), capabilities=[capability]
        ).run("Write")
...
class TestARunNobodyOrchestrates:
    ...
    async def test_delegates_to_an_a2a_subagent_as_it_always_did(
        ...
        async def relayed(
            self: Any, definition: Any, subagent_name: str, tool_call_id: Any, task: str
        ) -> str:
