...
def _get_base_dir() -> Path:
    ...
    if _BASE_DIR is None:
        # Try CWD first, fall back to system temp directory
        cwd = Path.cwd()
        if cwd == Path("/") or not os.access(cwd, os.W_OK):
            _BASE_DIR = Path(tempfile.gettempdir()) / "mcp_files"
        else:
            _BASE_DIR = cwd
...
def _normalize_path(path: str) -> Path:
    ...
    relative to the base directory (CWD or system temp dir/mcp_files if CWD is not writable).
    """
...
def generate_random_text(
    ...
    Args:
        ...
        seed: Optional random seed for reproducibility (deprecated, ignored for security).
    ...
    Note:
        ...
        The seed parameter is ignored as secrets module does not support seeding.
    """
    # Use secrets module for secure random selection
    words = [_WORDS[secrets.randbelow(len(_WORDS))] for _ in range(max(word_count, 0))]
    text = " ".join(words)
...
def read_text_file_many(
    ...
    for _ in range(times):
        last = read_text_file(
            path, include_content=include_content, max_chars=max_chars
        )
    return {
