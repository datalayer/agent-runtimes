...
1. **Per-tool policy approvals** for MCP/Skills (server + tool allowlists reflected in WebSocket snapshots).
2. **Per-call deferred approvals** for tools marked `requires_approval=True` (runtime stop-the-world approval/continue).
...
1. **Inline fast path (second-pass behavior)**
...
2. **Stop-the-world WebSocket path (unresolved only)**
