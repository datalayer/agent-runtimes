...
When you start the server with the [`--no-mcp-servers` CLI flag](/cli#mcp-server-control), no MCP servers are started automatically. You can then use the endpoints below to dynamically enable MCP servers from the catalog at runtime:
...
- You started the server with [`--no-mcp-servers`](/cli#mcp-server-control)
