...
:::note URL Convention
All API endpoints should be called **without trailing slashes**. For example:
- ✅ `/api/v1/agents`
- ❌ `/api/v1/agents/`

The only exception is A2A protocol endpoints which require trailing slashes for FastA2A compatibility.
:::
