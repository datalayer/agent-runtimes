...
The commands for teams of agents — list and show the catalogued teams, start
one on a goal, follow, steer, pause, resume, cancel or terminate its run — are
the `agent-teams` package's, not this one's: `agent-teams …` as its own
executable, and `datalayer agent-teams …` wherever it is installed beside the
Datalayer CLI. See the [agent-teams CLI](https://agent-teams.datalayer.tech/cli).
