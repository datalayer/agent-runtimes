...
The interactive assistant experience is part of Agent Runtimes and runs through `agent-runtimes cli`.
...
# Disable codemode
agent-runtimes cli --agentspec-id data-acquisition --no-codemode
```
...
```bash
agent-runtimes cli "What is Python?"
agent-runtimes cli -a crawler "Search for AI trends"
```
...
# ACP (WebSocket)
agent-runtimes cli connect ws://localhost:8000/api/v1/acp/ws/cli -t acp
```
...
```bash
agent-runtimes cli --banner
agent-runtimes cli --banner-all
agent-runtimes cli --eggs
```
...
```python
from agent_runtimes.codeai.cli import agent
...
- `agent_runtimes/codeai/cli.py`: Typer commands, runtime process management, and transport wiring
- `agent_runtimes/codeai/tux.py`: Rich terminal UX, slash command handling, and streaming rendering
- `agent_runtimes/codeai/commands/`: slash command modules and registry
- `agent_runtimes/codeai/banner.py`: brand output and animation helpers

### Runtime Flow

1. `agent-runtimes cli` resolves an agent spec (provided or interactive picker).
...
- ACP (`WebSocket`) is supported via `agent-runtimes cli connect -t acp`.

### Slash Command Registry

Slash commands are modularized in `agent_runtimes/codeai/commands/` and assembled via `build_commands(...)`. This keeps command behavior isolated and easy to evolve.

### Packaging

CLI-specific dependencies are defined in `agent-runtimes[cli]`:

```bash
pip install "agent-runtimes[cli]"
```

The primary user entry point is the `agent-runtimes cli` subcommand from the main `agent-runtimes` CLI.
