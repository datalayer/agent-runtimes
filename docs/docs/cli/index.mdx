...
For interactive assistant documentation (`agent-runtimes chat`), see the [Chat page](/chat).
...
### Kubernetes / Sidecar Environment Variables

These variables are useful when running `agent-runtimes` in Kubernetes with companion containers or remote sandboxes.

| Variable | Default | Description |
|----------|---------|-------------|
| `DATALAYER_RUNTIME_JUPYTER_SIDECAR` | `""` | Set to `"true"` when `agent-runtimes` runs alongside a Jupyter sidecar in the same pod. Enables sidecar-specific sandbox remapping and delays codemode initialization until Jupyter URL is provided. |
| `DATALAYER_CODE_SANDBOX_VARIANT` | `""` | Active sandbox variant (`local-eval`, `local-jupyter`, or `jupyter`) injected into sandboxes. |
| `DATALAYER_CODE_SANDBOX_URL` | `""` | Jupyter server URL (without token) injected into sandboxes. |
| `AGENT_RUNTIMES_MCP_PROXY_URL` | `""` | HTTP proxy URL for MCP tool calls from within sandboxes, for example `http://127.0.0.1:8765/api/v1/mcp/proxy`. |
| `AGENT_RUNTIMES_SKILLS_FOLDER` | `./skills` | Skills directory path. In Kubernetes this is typically `/mnt/shared-agent/skills`. |
