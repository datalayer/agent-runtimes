...
```python
from agent_runtimes.cli.cli import agent
...
- `agent_runtimes/cli/cli.py`: Typer commands, runtime process management, and transport wiring
- `agent_runtimes/cli/tux.py`: Rich terminal UX, slash command handling, and streaming rendering
- `agent_runtimes/cli/commands/`: slash command modules and registry
- `agent_runtimes/cli/banner.py`: brand output and animation helpers
...
Slash commands are modularized in `agent_runtimes/cli/commands/` and assembled via `build_commands(...)`. This keeps command behavior isolated and easy to evolve.
