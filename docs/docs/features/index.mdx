...
Loop is the browser workspace: a conversation, the editors the agent works
in, and the sandbox the code runs on — assembled entirely from plugins on the
Datalayer reactor. The shell is blank; the chat, the notebook, the document,
the command palette and even the theme arrive as plugin contributions, each
with first-class commands and keybindings.

See [Loop](/loop) for the architecture, the plugin graph, and how to embed or
extend it.
...
See [Protocols](/protocols) for protocol details.
...
- 🔁 **Loop Workspace** — a plugin-assembled workspace: chat, editors, sandbox, command palette
...
See the [Agentspecs](/agentspecs/) guide and [agentspecs.datalayer.tech](https://agentspecs.datalayer.tech) for details on cloning, generation, and specification authoring.
...
- [Sandbox Status WebSocket](/endpoints#sandbox-status-websocket)
- [Tool Approvals / Monitoring Stream](/endpoints#tool-approvals--monitoring-stream-websocket)
- [ACP Protocol WebSocket](/endpoints#acp-protocol-websocket)
...
See the [Protocols](/protocols) documentation for integration guides.
