...
- **An adapter reports; it never decides.** What an observation means to an
  ...
  work it never did.
- **An adapter says out loud what it cannot do.** A2A cannot steer a running
  task and ACP can; neither can pause, resume or checkpoint. Each adapter
  declares its own reductions and they reach the execution's event stream as
  the first thing a dispatch reports, so "why did this never checkpoint?" is
  answered from the run's own record.
