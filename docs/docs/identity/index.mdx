...
4. A file `kaggle.json` will download containing your credentials:
   ...
   ```
5. The `key` value is your `KAGGLE_API_TOKEN`
...
4. **Click "Get API Key"** — Opens [kaggle.com/settings/account](https://www.kaggle.com/settings/account) in a new tab
...
4. **Context variable set** — `set_request_identities()` stores tokens in a `contextvars.ContextVar`
5. **Tools access tokens** — `get_identity_env()` returns `{"KAGGLE_API_TOKEN": "your-key"}`

```python
# Inside your skill or codemode execution:
import os
kaggle_token = os.environ.get("KAGGLE_API_TOKEN")  # Available during execution
```
...
```bash
# Frontend .env
VITE_KAGGLE_API_TOKEN=your-api-key-from-kaggle-json
```
...
<AgentRuntimeFormExample
  identityProviders={{
    kaggle: {
      type: 'token',
      token: import.meta.env.VITE_KAGGLE_API_TOKEN,
      displayName: 'Kaggle',
    },
  }}
/>
...
```bash
# Backend .env
KAGGLE_API_TOKEN=your-api-key-from-kaggle-json
```
...
| **Authorization callback URL** | `http://localhost:3000/examples.html` | **Critical**: Must match the exact page URL |
...
|-------------|--------------|
| Agent Runtimes examples | `http://localhost:3000/examples.html` |
...
async function connectToMcpServer(datalayerMcpServerUrl: string) {
...
The [A2A (Agent-to-Agent) protocol](https://google.github.io/a2a/) includes identity provisions for inter-agent communication, complementing the runtime protocol layer documented in [Protocols](/protocols).
