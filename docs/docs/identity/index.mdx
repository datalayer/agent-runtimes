...
5. **Skill execution**: OAuth tokens must flow securely from the frontend to skill script execution
...
│  ┌─────────────────┐  ┌─────────────────┐  ┌─────────────────────┐  │
│  │  Token Manager  │  │  Agent Context  │  │  Skill Executor     │  │
...
## Identity in Skill Execution

When agents execute skills that require access to external services (like GitHub repositories), the OAuth tokens obtained via the identity system are automatically made available to skill scripts.

### How It Works

The identity-to-skill flow works as follows:

```
┌─────────────────────────────────────────────────────────────────────────────┐
│                         Identity → Skill Execution Flow                      │
└─────────────────────────────────────────────────────────────────────────────┘
...
Backend (Python):
  ...
  8. During skill execution, executor calls get_identity_env()
  9. Returns {"GITHUB_TOKEN": "..."} merged into subprocess environment
  10. Skill script accesses via os.environ["GITHUB_TOKEN"]
```
...
│                              Backend                                         │
│  ┌─────────────────┐     ┌─────────────────┐     ┌─────────────────────┐   │
│  │ vercel_ai.py    │────▶│ Identity Context│────▶│ LocalPythonExecutor │   │
│  │ (extract ids)   │     │ (contextvars)   │     │ (env vars)          │   │
│  └─────────────────┘     └─────────────────┘     └──────────┬──────────┘   │
└──────────────────────────────────────────────────────────────┼──────────────┘
                                                               │
                                                    subprocess with env
...
Both `LocalPythonExecutor` and `SandboxExecutor` automatically check the identity context:
...
- **Tokens are passed via environment variables** in the subprocess, not command-line arguments
- **Context variables are request-scoped** - they're cleared after the request completes
- **Subprocess isolation** - each skill execution gets a fresh subprocess with only the needed tokens
- **No persistent storage** - tokens are not written to disk during skill execution
:::

:::tip Principle of Least Privilege
Only tokens for connected providers are passed. If a user hasn't connected GitHub, `GITHUB_TOKEN` won't be set. Skills should gracefully handle missing tokens:

```python
token = os.environ.get("GITHUB_TOKEN")
if not token:
    print("Please connect your GitHub account to use this skill.")
    sys.exit(1)
```
:::
