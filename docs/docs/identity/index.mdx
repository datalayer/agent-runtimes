...
Kaggle MCP server supports OAuth 2.0 authentication, allowing agents to access Kaggle resources on behalf of users.

:::info Kaggle MCP Authorization
The [Kaggle MCP Server](https://www.kaggle.com/docs/mcp) follows the [MCP Authorization Specification](https://modelcontextprotocol.io/specification/2025-03-26/basic/authorization) based on OAuth 2.1. Authentication is handled automatically by the MCP client—no manual OAuth app registration required.
:::
...
|--------|----------------|----------|
| **OAuth 2.0** | None (automatic) | Interactive use, full access |
| **API Token** | Generate token in Kaggle settings | Automated/headless use |

#### Option 1: OAuth Authentication (Recommended)

When using OAuth, the MCP client handles authentication automatically:

1. First tool call triggers OAuth flow
2. Browser opens to Kaggle login/consent page
3. User authorizes the application
4. Tokens are stored and refreshed automatically

No client ID or client secret configuration is needed—Kaggle MCP handles this internally.

**Configuration in Agent Runtimes:**
...
For non-interactive or automated use, generate a Kaggle API token:

**Step 1: Generate Token**

1. Go to [Kaggle Settings](https://www.kaggle.com/settings)
2. Scroll to **API** section
3. Click **Create New Token** or **Generate New Token**
4. Copy the token (starts with `KGAT`)

**Step 2: Configure Environment**

```bash
# Add to your .env file or shell profile
export KAGGLE_TOKEN=KGAT...
```
...
#### Kaggle Identity in Agent Runtimes UI

To enable Kaggle identity management in the Agent Runtimes chat UI:
...
function App() {
  return (
    <IdentityConnect
      providers={{
        kaggle: {
          clientId: import.meta.env.VITE_KAGGLE_CLIENT_ID,
          scopes: ['datasets', 'notebooks', 'competitions'],
        },
      }}
    />
  );
...
:::tip Scopes
Kaggle OAuth scopes control access to different resources:
- `datasets` — Access to datasets
- `notebooks` — Create and manage notebooks
- `competitions` — Submit to competitions
- `models` — Access Kaggle models
:::
...
👉 **Use a GitHub App** (recommended for agent-style use cases)
...
|---------|--------------|-------|
| **PKCE** | ✅ Yes | Protects against authorization code interception attacks |
| **DCR** | ✅ Yes | For providers that support it (Okta, Auth0, Keycloak) |
| **Device Flow** | ❌ No | Not needed—we use browser-based redirect/popup flow |

:::caution DCR Provider Support
**Most consumer OAuth providers do NOT support DCR:**
- ❌ GitHub - manual registration required
- ❌ Google - manual registration required  
- ⚠️ Azure AD - tenant-admin only
...
:::
| **Dynamic Client Registration (DCR)** | ✅ Yes | Register clients dynamically with OAuth providers |
| **Device Flow** | ❌ No | Not needed—we use browser-based redirect/popup flow |
...
GitHub OAuth Apps **require a client secret** for the token exchange, even when using PKCE. You must generate and configure the client secret (see Step 4 below).
:::
...
:::warning Client Secret IS Required for GitHub
**GitHub OAuth Apps require a client secret** for the token exchange, even when using PKCE.

While OAuth 2.1 with PKCE is designed for public clients, GitHub's token endpoint requires the client secret. The secret is stored securely on the backend (never in the browser).

| Provider | Client Secret | Notes |
|----------|---------------|-------|
| **GitHub** | ✅ Required | Token exchange requires secret |
| **Google** | ✅ Required | Same as GitHub |
| **Okta/Auth0** | ❌ Optional | Pure PKCE flows supported |

**You must set both `GITHUB_CLIENT_ID` and `GITHUB_CLIENT_SECRET`** in your `.env` file for the backend.
:::
...
:::caution GitHub Requires Client Secret
GitHub's token endpoint requires a client secret even with PKCE. The Agent Runtimes backend performs the token exchange (to avoid CORS issues), so you **must configure the client secret**.
...
# Required for GitHub OAuth token exchange
GITHUB_CLIENT_ID=your_client_id
GITHUB_CLIENT_SECRET=your_client_secret  # Required for GitHub!
```
...
OAUTH_PROVIDERS = {
    "github": {
        ...
        "client_secret": os.getenv("GITHUB_CLIENT_SECRET"),  # Required for GitHub
...
| **Google** | ❌ No | Manual registration via Google Cloud Console |
...
|----------------|-------------|----------|
| **Skills** | Standalone Python scripts executed via `LocalPythonExecutor` or `SandboxExecutor` | Predefined, reusable automation scripts |
...
│                               │ (LocalPython/       │ │ (execute_code)  │  │
│                               │  Sandbox)           │ │                 │  │
│                               └──────────┬──────────┘ └────────┬────────┘  │
└──────────────────────────────────────────┼─────────────────────┼───────────┘
                                           │                     │
                              subprocess with env    sandbox with env
                                           │                     │
                              ┌────────────▼────────┐ ┌──────────▼──────────┐
...
```python
from agent_skills import LocalPythonExecutor, SandboxExecutor

# LocalPythonExecutor - identity context enabled by default
executor = LocalPythonExecutor()

# To disable identity context (e.g., for untrusted scripts)
executor = LocalPythonExecutor(use_identity_context=False)

# SandboxExecutor - also supports identity context
from code_sandboxes import LocalEvalSandbox
sandbox = LocalEvalSandbox()
executor = SandboxExecutor(sandbox)  # Uses run_code with envs parameter
```
