...
2. Keep configuration source-of-truth **local** on each node.
...
3. If upstream returns no models, Agent Node applies Bedrock Anthropic
   fallback defaults so the UI can still present usable choices.
...
2. **Configuration** — pick mode, billable account, and sharing policy.
   ...
   models returned by `/api/v1/configure/inference/models`.
...
`/api/ai-inference/v1/ping` readiness.
...
2. **Visibility regressions** — explicit policy tests for own/shared-only
   queries.
3. **Mode migration drift** — temporary alias support (`run`/`host`) with
   strict deprecation window.
