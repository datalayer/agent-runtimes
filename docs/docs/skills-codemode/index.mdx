---
title: Skills & Codemode
---

# Skills & Codemode

## What this page covers
- Why we offer two capabilities: reusable [Skills](https://agent-skills.datalayer.tech/) and programmatic [Codemode](https://mcp-codemode.datalayer.tech/)
...
- **Enable Skills**: adds `DatalayerSkillsToolset` (skills discovery, load, resource read, script run). Uses the configured `skills/` directory.
- **Enable Codemode**: adds `CodemodeToolset` (list/search tools, execute Python code). When Codemode is on, MCP server selection in the UI is disabled because Codemode handles discovery itself.
...
- `search_tools` (returns tool definitions; can be reranked)
- `get_tool_details` (full schema for one tool)
...
- Errors are caught and surfaced in the `execute_code` result payload.

## Skills model
- Skills live in a `skills/` directory with `SKILL.md`, resources, and scripts.
- Tooling provided by `DatalayerSkillsToolset`: `list_skills`, `load_skill`, `read_skill_resource`, `run_skill_script`.
...
## UI & backend flow
- UI checkboxes toggle Skills/Codemode. If Codemode is enabled, MCP server selection is disabled (Codemode handles discovery).
- Backend route `/api/v1/agents` builds toolsets accordingly:
	- Skills enabled → add `DatalayerSkillsToolset`.
