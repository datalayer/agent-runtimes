...
For WebSocket endpoints, switch from `http://` to `ws://` (or `wss://` in TLS environments).

```text
ws://localhost:8765/api/v1
```
...
|-------|---------|-----------------|----------------|
| **1 — Startup** | Dockerfile CMD (`--codemode`, no `--jupyter-sandbox`) | `eval` | In-process `eval()` (fallback) |
| **2 — Companion call** | `POST /api/v1/agents/mcp-servers/start` with `jupyter_sandbox` + `mcp_proxy_url` | `jupyter` | Jupyter kernel (`:2300`) |

**Phase 1**: The agent-runtimes container starts with `--codemode` and `--no-catalog-mcp-servers`. Because no `--jupyter-sandbox` flag is provided, the `CodeSandboxManager` defaults to `variant="eval"`.
...
The endpoint calls `CodeSandboxManager.configure_from_url()`, which switches the variant to `jupyter` and rebuilds the Codemode toolset. All subsequent code execution runs inside the Jupyter kernel instead of local eval.
...
| `jupyter_sandbox` | string | No | Jupyter server URL with token for code execution. When provided, switches the sandbox from `eval` to `jupyter` (Phase 2 of the bootstrap) |
...
```json
{
  ...
  "message": "Started 2 server(s) across 2 agent(s), sandbox=jupyter"
}
```
...
| `sandbox_variant` | string | The sandbox variant after configuration (`eval` or `jupyter`) |
...
- **`eval`**: Uses Python `exec()` for simple code execution (default)
- **`jupyter`**: Connects to an external Jupyter server for persistent kernel state
...
| Field | Type | Description |
|-------|------|-------------|
| `variant` | string | Current sandbox variant (`eval` or `jupyter`) |
...
```http
GET /api/v1/configure/sandbox/ws?agent_id={agent_id}
```

Streams sandbox execution state and accepts interrupt requests.
...
| Field | Type | Description |
|-------|------|-------------|
| `variant` | string | `eval`, `jupyter`, `unavailable`, or `error` |
| `sandbox_running` | boolean | Whether sandbox exists and is ready |
| `is_executing` | boolean | Whether sandbox is currently executing code |
| `jupyter_url` | string/null | Effective Jupyter URL when applicable |

**Client -> Server interrupt request:**
...
**Server -> Client interrupt response:**
...
When `variant` is `jupyter`:
- If `jupyter_url` is provided, agent-runtimes connects to that existing Jupyter server.
- If `jupyter_url` is omitted, agent-runtimes uses a managed local Jupyter sandbox.
...
| Field | Type | Required | Description |
|-------|------|----------|-------------|
| `variant` | string | No | Sandbox variant: `eval` (default) or `jupyter` |
| `jupyter_url` | string | No | Optional Jupyter server URL (can include token as query param). If omitted with `variant=jupyter`, a managed local Jupyter sandbox is used. |
| `jupyter_token` | string | No | Jupyter token (overrides token in URL if provided) |

**Example Requests:**

```bash
# Configure for managed local Jupyter sandbox
curl -X POST http://localhost:8000/api/v1/agents/sandbox/configure \
  -H "Content-Type: application/json" \
  -d '{"variant": "jupyter"}'

# Configure for external Jupyter server
curl -X POST http://localhost:8000/api/v1/agents/sandbox/configure \
  -H "Content-Type: application/json" \
  -d '{"variant": "jupyter", "jupyter_url": "http://localhost:8888?token=my-token"}'

# Reset to eval sandbox
curl -X POST http://localhost:8000/api/v1/agents/sandbox/configure \
  -H "Content-Type: application/json" \
  -d '{"variant": "eval"}'
```
...
### Tool Approvals / Monitoring Stream (WebSocket)

```http
GET /api/v1/tool-approvals/ws?agent_id={agent_id}
```

Compatibility aliases are also available:

- `/api/ai-agents/v1/tool-approvals/ws`
- `/api/ai-agents/v1/ws`

This stream sends a versioned envelope. The first message is always an
`agent.snapshot`, followed by incremental events and refreshed snapshots.
...
```json
{ "type": "skill_enable", "skillId": "github" }
```

```json
{ "type": "skill_disable", "skillId": "github" }
```

### ACP Protocol WebSocket

```http
GET /api/v1/acp/ws/{agent_id}
```

Bidirectional ACP transport over WebSocket. This endpoint expects ACP message
objects (for example `initialize`, `session/new`, `session/prompt`, etc.) and
returns ACP-compliant responses/events.

If `{agent_id}` is unknown, the server sends an ACP error payload and closes
the socket.

The route speaks the ACP schema:

- `session/prompt` takes the prompt's content blocks under `prompt`, and
  answers with a `stopReason` once the turn is over.
- A prompt may carry a model budget in its `_meta`:
  `{"datalayer": {"budget": {"inputTokens": …, "outputTokens": …, "cost": …}}}`,
  which an orchestration delegation sends. The turn is held to it as
  pydantic-ai's per-run usage limits; a limit reached ends the turn with
  `max_tokens` (or `max_turn_requests` for requests), and `_meta` in the answer
  names it: `{"datalayer": {"error": {"code": "budget_exhausted", "details":
  {"budget": "model", "limit": "output_tokens"}}}}`.
- A prompt an orchestration delegation sends also carries the execution's own
  token in `_meta`, as `{"datalayer": {"credential": "…"}}`. The route takes it
  out of `_meta` before anything keeps the prompt, and the turn runs as that
  token — its approvals, its telemetry and its Datalayer MCP toolset: the
  agent's Datalayer server is replaced, for the turn, by one holding the token
  (`agent_runtimes.mcp.datalayer_gateway`), so what the gateway lets the turn
  reach is what the execution's manifest names. An agent given no Datalayer
  server is not given one by a delegation.
- Each `session/update` nests the update under `update`, its text in a content
  block — `{"sessionId": "…", "update": {"sessionUpdate": "agent_message_chunk",
  "content": {"type": "text", "text": "…"}}}`; tool calls are `tool_call` and
  `tool_call_update` with `toolCallId`, `title`, `rawInput`, `rawOutput` and
  `status`. A failed turn is the prompt's JSON-RPC error.
- `session/cancel` is a notification: the running prompt stops where the agent
  is waiting and answers `cancelled`; the cancel itself is not answered. A
  prompt runs on its own task, so the cancel is read while it runs.
- `initialize` declares `loadSession: true`. Sessions and each turn of their
  conversation are kept in the runtime's protocol state store, so
  `session/load` on a restarted runtime finds the session and replays its
  conversation as `user_message_chunk` and `agent_message_chunk` updates before
  answering.

The A2A agents mounted under `/api/v1/a2a/agents/{agent_id}` keep their tasks,
contexts and owed runs in the same store: `tasks/get` answers for a task issued
before a restart, and a task left submitted or working is run again when the
runtime starts. A message's `metadata` may carry the same `datalayer.budget` as
an ACP prompt's `_meta`; a run it stops ends the task `failed`, and the failed
status message's `metadata` carries the same `datalayer.error`. It may carry
the same `datalayer.credential` too. The store takes it out of the message
before the task, its context or the owed run is written, keeps `withheld` in
its place, and holds the credential in process memory for the run it came
with; a run a restarted runtime owes has lost it and fails rather than run as
the runtime, and the execution's retry dispatches with a token of its own.

Both routes speak the Datalayer orchestration extension,
`https://datalayer.ai/extensions/orchestration/v1` — declared on the A2A agent
card with its routes as params, and at ACP `initialize` under
`agentCapabilities._meta.datalayer.extensions`:

- A delegation names its execution, `datalayer.execution: {executionId, …}`,
  which is the scope its checkpoints are kept under, and may name a checkpoint
  to resume from, `datalayer.checkpoint: {checkpointId}`: the run starts from
  that checkpoint's conversation. A checkpoint this runtime does not keep fails
  the A2A task, and is refused `-32602` over ACP.
- `POST /api/v1/a2a/pause {task_id}`, or ACP `session/cancel` with
  `_meta: {"datalayer": {"pause": true}}`, pauses a running turn: it saves its
  conversation so far as a checkpoint in the protocol state store and ends as a
  cancel naming it — the A2A task `canceled` with `datalayer.paused.checkpointId`
  in its final status message's `metadata`, the ACP turn `cancelled` with the
  same in `_meta`. A run that names no execution is only stopped.
- `POST /api/v1/a2a/steer {task_id, instructions}`, or the ACP notification
  `_datalayer/steer {sessionId, instructions}`, adds instructions to a running
  turn: they join its next model request. A run that is not working receives
  nothing, and the A2A route answers `success: false`.
- Each turn answers with what it spent, `datalayer.usage: {inputTokens,
  outputTokens, cost, currency}`: in the ACP prompt response's `_meta`, beside
  `datalayer.paused` or `datalayer.error` when the turn paused or its budget
  stopped it, and in the `metadata` of the A2A task's final status message,
  which the worker sends itself for every ending — `completed` with the answer,
  `canceled` and `failed` alike. The tokens are the model's own count when the
  run reports one, and otherwise what the agent's usage counters grew by during
  the turn; `cost` is there only when the agent's cost monitor priced a run
  during it. A turn that counted nothing sends no `usage`.
- A run whose delegation names its execution is that execution's worker, and
  `datalayer.execution.accountUid` names the account the execution belongs to.
  Such a run does not take a child itself. `delegate_task` to a subagent
  reached over A2A asks the control plane for a child execution under the
  run's own, with the run's credential, and answers with what the child
  produced; the child's progress arrives as `agent.subagent` events. Resolving,
  launching or relaying to an agent from inside the run is refused. In-process
  subagents stay in the run.

The store is a SQLite file at
`AGENT_RUNTIMES_PROTOCOL_STATE_PATH` (default
`~/.datalayer/agent-runtimes/protocol-state.sqlite`), or, on Kubernetes, the
agent-memories PostgreSQL the memory backends use.
...
Agent Node operations are split between:

- **Local node runtime endpoints** on `agent-runtimes` (`/api/v1/...`)
- **Central control-plane endpoints** on `datalayer-runtimes` (`/api/runtimes/v1/...`)

### Local Agent Node Configuration (agent-runtimes)
...
Read or update local node configuration.

**Configuration fields:**

- `mode`: `private`, `shared`, or `sleep`.
- `billing_entity_uid`
- `billing_entity_type`
- `billing_entity_handle`
- `sharing` (JSON object)

### Central Agent Node Registry (datalayer-runtimes)
...
POST /api/runtimes/v1/agent-nodes/aws/discover
GET  /api/runtimes/v1/agent-nodes
GET  /api/runtimes/v1/agent-nodes/{node_id}
```

These endpoints maintain fleet state and enforce visibility:

- own nodes are always visible,
- other users' nodes are visible only when `mode=shared`.

`POST /agent-nodes/aws/discover` accepts `{ "aws_account_id": "123456789012" }`,
allowlists that account id for the caller in memory, and returns currently
available nodes registered with the same AWS account id.

### Agent Node Tunnel (datalayer-runtimes)
...
The tunnel provides bidirectional routing for central chat:

- UI -> runtimes -> selected node (`chat.request`)
- node -> runtimes -> UI (`chat.response` / `chat.error`)

The websocket endpoint requires authenticated query parameters (`node_id` and
`token`) and keeps a persistent node session.
