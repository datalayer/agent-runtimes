...
The Datalayer TypeScript SDK (`@datalayer/agent-runtimes`) provides a comprehensive client library for building modern web applications and integrations with the Datalayer AI Platform. This SDK offers TypeScript-first APIs, React components, utilities, and hooks for seamless development.
...
```bash
npm install @datalayer/agent-runtimes
```

```bash
yarn add @datalayer/agent-runtimes
```
...
```typescript
import { useCoreStore, useUser } from '@datalayer/agent-runtimes';
import { PlatformRoles } from '@datalayer/agent-runtimes/lib/models';
...
```typescript
import { useIAMStore } from '@datalayer/agent-runtimes';
...
```typescript
import { useRuntimesStore } from '@datalayer/agent-runtimes';
import { RuntimeSimplePicker } from '@datalayer/agent-runtimes/lib/components/runtimes';
...
import { 
  ...
  useRuntimesStore,
  useLayoutStore 
} from '@datalayer/agent-runtimes/lib/state';
...
```typescript
import { DatalayerApi } from '@datalayer/agent-runtimes/lib/api';
...
import { ToastContainer } from '@datalayer/agent-runtimes/lib/components';
import { CoreProvider } from '@datalayer/agent-runtimes/lib/context';
...
- Explore the [TypeScript API](../typescript_api) for detailed type definitions and function references
