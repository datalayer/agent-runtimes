...
- ✅ Loop — the plugin-based browser workspace (chat + editors + sandbox)
...
For agent and provider specifications, see [agentspecs.datalayer.tech](https://agentspecs.datalayer.tech) and [Agentspecs](/agentspecs/).
...
|---------|---------------|
| The browser workspace (plugins, editors, commands) | [Loop Web](/loop) |
| The interactive terminal | [Loop REPL](/loop-repl) |
| UX patterns (GenUI) with A2UI and AG-UI | [Protocols](/protocols), [Chat](/chat) |
| Interactive or triggered workflows | [Agentspecs](/agentspecs), [Endpoints](/endpoints) |
| Agent Identity and Controls (guardrails, monitoring, tool approvals) | [Identity](/identity), [Endpoints](/endpoints#tool-approvals--monitoring-stream-websocket) |
| Programmatic tooling with Sandbox and Codemode for MCP and Skills | [Programmatic Tools](/programmatic-tools), [Hooks](/hooks), [Endpoints](/endpoints#sandbox-status-websocket) |
| Outputs and Notifications | [Integrations](/integrations), [Agentspecs](/agentspecs) |
| Real-time collaboration with users, subagents, and multi-agent teams | [Protocols](/protocols), [Endpoints](/endpoints#acp-protocol-websocket), [Agentspecs](/agentspecs) |
| Custom agents built from Agentspecs | [Agentspecs](/agentspecs), [Loop CLI](/cli) |
