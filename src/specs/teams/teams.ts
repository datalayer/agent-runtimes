...
export const JUPYTER_NOTEBOOK_TEAM_SPEC_0_0_1: TeamSpec = {
  id: 'jupyter-notebook',
  version: '0.0.1',
  name: 'Jupyter Notebook',
  description: `A team for working in a notebook: one member teaches the person Python in the notebook they have open, the other rewrites that notebook as short as it can be without changing what it computes.`,
  ...
  routingInstructions: `Send anything about understanding, learning or explaining — "what does this do", "why did this fail", "teach me", a request for an exercise — to the Tutor. Send anything about the shape of the notebook — shortening it, merging cells, removing dead code or stale outputs — to the Compactor. When a request is both ("clean this up and explain what you did"), the Tutor goes first and explains, then the Compactor acts on what was agreed. Never let the Compactor rewrite a notebook the person is still learning from without saying what changed. When neither fits, answer directly rather than forcing the request onto a specialist.`,
  suggestions: [
    {
...
export const SHARED_NOTEBOOK_TEAM_SPEC_0_0_1: TeamSpec = {
  id: 'shared-notebook',
  version: '0.0.1',
  name: 'Shared Notebook',
  description: `Analyst, Reviewer, Writer and Decks, working in the same Jupyter notebook: the Analyst adds and runs the cells, the Reviewer checks them and records what it finds, the Writer writes the result up between them, and Decks turns what they found into a presentation.`,
  tags: [
    'notebook',
    'jupyter',
    'collaboration',
  ...
  supervisor: {
    ...
    model: 'bedrock:us.anthropic.claude-sonnet-4-6',
    goal: `Keep the work in the notebook. Answer analysis requests directly; send a request to check something to the Reviewer, a request to write something up to the Writer and a request for slides to Decks, and never leave results anywhere but the cells — or the deck made from them.`,
    instructions: ``,
    approval: 'auto',
    canTerminate: false,
  },
  routingInstructions: `Send anything about exploring, computing, plotting or extending the analysis — "analyze", "plot", "compute", "add a cell that…" — to the Analyst. Send anything about checking, verifying, doubting or re-running — "find what is wrong", "check this", "is this right", "re-run" — to the Reviewer. Send anything about explaining, summarising, writing up or verifying the wording of a conclusion to the Writer. Send anything about slides, a deck or a presentation — "make a deck of this", "turn the findings into slides", "present this", "add a slide" — to Decks, which builds the deck from what the cells show and opens it beside the conversation. When a request is two of these ("analyze this and write it up", "analyze this and make a deck"), the Analyst goes first and the Writer or Decks follows on the cells it produced. When none fits, answer directly rather than forcing the request onto a specialist.`,
  suggestions: [
...
export const TEAM_SPECS: Record<string, TeamSpec> = {
  ...
  'jupyter-notebook': JUPYTER_NOTEBOOK_TEAM_SPEC_0_0_1,
  ...
  'shared-notebook': SHARED_NOTEBOOK_TEAM_SPEC_0_0_1,
