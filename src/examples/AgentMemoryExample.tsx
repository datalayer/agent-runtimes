...
/**
 * Agent Memory, demonstrated as a Loop capacity plugin.
 *
 * The capacity lives in `@datalayer/loop-plugin-agent-memory`: the
 ...
 *
 * @module examples/AgentMemoryExample
 */
...
const AgentMemoryExample: React.FC = () => {
  ...
  const agentName = useMemo(() => uniqueAgentId('memory-example-agent'), []);
  const plugins = useMemo(() => [AgentMemoryPlugin], []);
  return (
    <ThemedProvider>
