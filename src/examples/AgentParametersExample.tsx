...
/**
 * Agent Parameters, demonstrated as a Loop capacity plugin.
 *
 * The capacity lives in `@datalayer/loop-plugin-agent-parameters`: the
 ...
 *
 * @module examples/AgentParametersExample
 */
...
const AgentParametersExample: React.FC = () => {
  ...
  const agentName = useMemo(() => uniqueAgentId('parameters-example-agent'), []);
  const plugins = useMemo(() => [AgentParametersPlugin], []);
  return (
    <ThemedProvider>
