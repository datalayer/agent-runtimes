...
/**
 * Agent Guardrails, demonstrated as a Loop capacity plugin.
 *
 * The capacity lives in `@datalayer/loop-plugin-agent-guardrails`: the
 ...
 *
 * @module examples/AgentGuardrailsExample
 */
...
const AgentGuardrailsExample: React.FC = () => {
  ...
  const agentName = useMemo(() => uniqueAgentId('guardrails-example-agent'), []);
  const plugins = useMemo(() => [AgentGuardrailsPlugin], []);
  return (
    <ThemedProvider>
