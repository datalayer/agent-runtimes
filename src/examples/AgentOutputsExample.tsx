...
/**
 * Agent Outputs, demonstrated as a Loop capacity plugin.
 *
 * The capacity lives in `@datalayer/loop-plugin-agent-outputs`: the
 ...
 *
 * @module examples/AgentOutputsExample
 */
...
const AgentOutputsExample: React.FC = () => {
  ...
  const agentName = useMemo(() => uniqueAgentId('outputs-example-agent'), []);
  const plugins = useMemo(() => [AgentOutputsPlugin], []);
  return (
    <ThemedProvider>
