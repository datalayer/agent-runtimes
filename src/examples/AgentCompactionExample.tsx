...
/**
 * Agent Compaction, demonstrated as a Loop capacity plugin.
 *
 * The capacity lives in `@datalayer/loop-plugin-agent-compaction`: the
 ...
 *
 * @module examples/AgentCompactionExample
 */
...
const AgentCompactionExample: React.FC = () => {
  ...
  const agentName = useMemo(() => uniqueAgentId('compaction-example-agent'), []);
  const plugins = useMemo(() => [AgentCompactionPlugin], []);
  return (
    <ThemedProvider>
