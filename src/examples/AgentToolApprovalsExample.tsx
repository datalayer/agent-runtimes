...
/**
 * Agent Tool Approvals, demonstrated as a Loop capacity plugin.
 *
 * The capacity lives in `@datalayer/loop-plugin-agent-tool-approvals`: the
 ...
 *
 * @module examples/AgentToolApprovalsExample
 */
...
const AgentToolApprovalsExample: React.FC = () => {
  ...
  const agentName = useMemo(() => uniqueAgentId('tool-approval-example-agent'), []);
  const plugins = useMemo(() => [AgentToolApprovalsPlugin], []);
  return (
    <ThemedProvider>
