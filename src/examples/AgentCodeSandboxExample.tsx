...
/**
 * Agent Code Sandbox, demonstrated as a Loop capacity plugin.
 *
 * The capacity lives in `@datalayer/loop-plugin-agent-code-sandbox`: the
...
const AgentCodeSandboxExample: React.FC = () => {
  ...
  const agentName = useMemo(() => uniqueAgentId('code-sandbox-example-agent'), []);
  const plugins = useMemo(() => [AgentCodeSandboxPlugin], []);
  return (
    <ThemedProvider>
