...
/**
 * Agent MCP, demonstrated as a Loop capacity plugin.
 *
 * The capacity lives in `@datalayer/loop-plugin-agent-mcp`: the
 ...
 *
 * @module examples/AgentMCPExample
 */
...
const AgentMCPExample: React.FC = () => {
  ...
  const agentName = useMemo(() => uniqueAgentId('mcp-example-agent'), []);
  const plugins = useMemo(() => [AgentMcpPlugin], []);
  return (
    <ThemedProvider>
