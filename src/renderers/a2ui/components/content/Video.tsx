...
interface VideoProps extends A2UIComponentProps {
  node: Types.AnyComponentNode & { type: 'Video' };
}

export function Video({ node, surfaceId }: VideoProps) {
  ...
  return (
    <video src={url} controls className={cn(themeClasses)} preload="metadata" />
  );
}
